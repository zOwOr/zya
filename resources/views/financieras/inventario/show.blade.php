...
<div class="container-fluid">
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                <div>
                    <div class="d-flex align-items-center">
                        <h4 class="mb-0 font-weight-bold mr-3">
                            <i class="fa-solid fa-mobile-screen text-primary mr-2"></i>Dispositivo IMEI: {{ $device->imei }}
                        </h4>
                ...
                <div class="mt-3 mt-md-0">
                    ...
                    <a href="{{ route('financieras.inventario.history', $device->id) }}" class="btn btn-outline-info btn-sm mr-1" title="Ver Historial">
                        <i class="fa-solid fa-clock-rotate-left mr-1"></i>Trazabilidad
                    </a>
                    ...
                    @if (auth()->user()->can('financieras.inventario.edit'))
                        <a href="{{ route('financieras.inventario.edit', $device->id) }}" class="btn btn-warning btn-sm">
                            <i class="fa-solid fa-pen-to-square mr-1"></i>Editar Dispositivo
                        </a>
                    @endif
                </div>
            </div>
            ...
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-header bg-white border-bottom py-3">
                    <h6 class="font-weight-bold mb-0 text-dark">
                        <i class="fa-solid fa-circle-info text-primary mr-2"></i>Ficha Técnica del Equipo
                    </h6>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-3 col-sm-6 mb-3">
                            <span class="text-muted font-size-12 font-weight-bold text-uppercase d-block">Número de IMEI</span>
                            <span class="font-weight-bold font-size-16 text-dark">{{ $device->imei }}</span>
                        </div>
                        <div class="col-md-3 col-sm-6 mb-3">
                            <span class="text-muted font-size-12 font-weight-bold text-uppercase d-block">Marca</span>
                            <span class="font-weight-bold font-size-15 text-dark">{{ $device->brand?->name ?? 'Sin Marca' }}</span>
                        </div>
                        <div class="col-md-3 col-sm-6 mb-3">
                            <span class="text-muted font-size-12 font-weight-bold text-uppercase d-block">Modelo</span>
                            <span class="font-weight-bold font-size-15 text-dark">{{ $device->model }}</span>
                        </div>
                        <div class="col-md-3 col-sm-6 mb-3">
                            <span class="text-muted font-size-12 font-weight-bold text-uppercase d-block">Color</span>
                            <span class="font-weight-bold font-size-15 text-dark">{{ $device->color ?: 'No especificado' }}</span>
                        </div>
                        <div class="col-md-3 col-sm-6 mb-3">
                            <span class="text-muted font-size-12 font-weight-bold text-uppercase d-block">Capacidad / RAM</span>
                            <span class="font-weight-bold font-size-15 text-dark">{{ $device->storage ?: 'No especificada' }}</span>
                        </div>
                        <div class="col-md-3 col-sm-6 mb-3">
                            <span class="text-muted font-size-12 font-weight-bold text-uppercase d-block">Sucursal Asignada</span>
                            <span class="font-weight-bold font-size-15 text-primary">{{ $device->branch?->name ?? 'Sin Sucursal' }}</span>
                        </div>
                        <div class="col-md-3 col-sm-6 mb-3">
                            <span class="text-muted font-size-12 font-weight-bold text-uppercase d-block">Proveedor</span>
                            <span class="font-weight-bold font-size-15 text-dark">{{ $device->supplier?->name ?? 'No especificado' }}</span>
                        </div>
                        <div class="col-md-3 col-sm-6 mb-3">
                            <span class="text-muted font-size-12 font-weight-bold text-uppercase d-block">Fecha de Llegada / Alta</span>
                            <span class="font-weight-bold font-size-15 text-dark">
                                {{ $device->arrived_at ? \Carbon\Carbon::parse($device->arrived_at)->format('d/m/Y') : ($device->created_at ? $device->created_at->format('d/m/Y') : '-') }}
                            </span>
                        </div>
                        @if ($device->notes)
                            <div class="col-12 mt-2">
                                <span class="text-muted font-size-12 font-weight-bold text-uppercase d-block">Notas / Observaciones</span>
                                <div class="p-2 bg-light rounded text-secondary font-size-13 border">
                                    {{ $device->notes }}
                                </div>
                            </div>
                        @endif
            ...
            <!-- Fichas de Historial y Movimientos -->
            <div class="row">
                            ...
                            @forelse ($device->sales as $s)
                                <div class="border rounded p-3 mb-2 bg-light">
                                    ...
                                    <div class="font-size-13 text-dark">
                                        ...
                                        <strong>Tipo:</strong> <span class="badge badge-{{ $s->isContado() ? 'success' : 'primary' }}">{{ $s->isContado() ? 'Contado' : ($s->financiera?->name ?? 'Crédito') }}</span> | <strong>Vendedor:</strong> {{ $s->seller?->name }}<br>
