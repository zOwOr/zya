...
@section('container')
    <div class="container">
        ...
        {{-- FORMULARIO DE BÚSQUEDA --}}
        <form method="GET" action="{{ route('cash.dailyCut') }}" class="mb-3">
            <div class="mb-2 text-muted" style="font-size: 0.9rem;">
                Puedes buscar por orden, usuario o método (usa "efectivo", "transferencia" o "tarjeta" para filtrar pagos).
            </div>
            <div class="input-group">
                <input type="text" name="search" value="{{ request('search') }}" class="form-control"
                    placeholder="Buscar por orden, usuario o método">
        ...
        {{-- TABLA DE PAGOS DE PROVEEDORES ESPECIALES --}}
        @if ($specialProviderPayments->count())
            <h5 class="mt-5">Pagos de Proveedores Especiales (Payjoy, KrediYa)</h5>
            <table class="table table-sm table-bordered">
                <thead class="table-light">
                    <tr>
                        <th>Fecha</th>
                ...
                <tbody>
                    @foreach ($specialProviderPayments as $payment)
                        <tr>
                            <td>{{ $payment->created_at->format('d/m/Y H:i') }}</td>
                            <td>#{{ $payment->order_id }}</td>
                            <td>${{ number_format($payment->amount, 2) }}</td>
                            ...
                            <td>{{ $methodNames[$payment->method] ?? $payment->method }}</td>
