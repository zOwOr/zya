                                    ...
                                    <div class="profile-img-edit">
                                        <div class="crm-profile-img-edit">
                                            <img class="crm-profile-pic rounded-circle avatar-100" id="image-preview"
                                                src="{{ $userData->photo ? asset('storage/profile/' . $userData->photo) : asset('assets/images/user/1.png') }}"
                                                alt="profile-pic">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="input-group mb-4 col-lg-6">
                                    <div class="custom-file">
                                        <input type="file" class="custom-file-input @error('photo') is-invalid @enderror"
                                            id="image" name="photo" accept=".pdf, image/*" onchange="previewImage();">
                                    ...
                                    <select class="form-control @error('branch_id') is-invalid @enderror" name="branch_id"
                                        ...
                                        @foreach ($branches as $branch)
                                            <option value="{{ $branch->id }}"
                                                {{ old('branch_id', $userData->branch_id) == $branch->id ? 'selected' : '' }}>
                                                {{ $branch->name }}
                                            </option>
                                        @endforeach
                                    </select>
